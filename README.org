...
- ~vertico-mouse~ and ~vertico-quick~ select the candidate shown on the
  corresponding carousel row.
- If another Vertico display mode such as ~vertico-flat-mode~,
  ~vertico-grid-mode~, or ~vertico-unobtrusive-mode~ is active, that display
  mode takes precedence over carousel display for the session.
